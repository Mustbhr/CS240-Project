...
class CheckpointReplicator:
    ...
    def __init__(self, rank: int, world_size: int, replication_factor: int = 2):
        ...
        logger.info(
            f"[GPU {rank}] CheckpointReplicator initialized with m={replication_factor}"
        )
    ...
    def store_replica(self, source_rank: int, iteration: int, checkpoint_bytes: bytes):
        ...
        logger.debug(
            f"[GPU {self.rank}] Stored replica from GPU {source_rank} iter {iteration}"
        )
    ...
    def cleanup_old_replicas(self, keep_iterations: List[int]):
        ...
        keys_to_remove = [key for key in self.replicas if key[1] not in keep_iterations]
...
class GeminiTrainer:
    ...
    def __init__(
        self, config: GeminiConfig, rank: int, world_size: int, local_rank: int
    ):
        self.config = config
        self.rank = rank
        self.world_size = world_size
    ...
    def save_checkpoint_gemini(
        self, model: nn.Module, optimizer: torch.optim.Optimizer, iteration: int
    ) -> Dict[str, float]:
        """
    ...
    def _replicate_checkpoint(self, iteration: int, checkpoint_bytes: bytes):
        """
        Replicate checkpoint to peer GPUs using point-to-point ring-based replication.

        For m=2 replication:
        - GPU 0 sends to GPU 1
        - GPU 1 sends to GPU 2
        - GPU 2 sends to GPU 3
        - GPU 3 sends to GPU 0

        This is much more efficient than all_gather which sends all checkpoints to all GPUs.
        """
        import numpy as np
        import gc
        ...
        if self.rank == 0:
            logger.info(
                f"--- [Replication] Start Iter {iteration} (Point-to-Point) ---"
            )
        ...
        if self.rank == 0:
            logger.info(
                f"[Replication] Sending {local_size / 1024**2:.2f} MB to GPU {send_target}, "
                f"receiving {recv_size / 1024**2:.2f} MB from GPU {recv_source}"
            )
        ...
        # Barrier to ensure all GPUs finish replication before continuing
        dist.barrier(device_ids=[self.local_rank])
        if self.rank == 0:
            logger.info(f"--- [Replication] Finished Iter {iteration} ---")
        ...
        while iteration < self.config.max_iterations:
            ...
            for batch in dataloader:
                ...
                if (
                    ...
                    and iteration == self.config.failure_iteration
                ):
                    logger.warning(
...
def main():
    ...
    parser.add_argument(
        "--max-iterations", type=int, default=200, help="Max iterations"
    )
    parser.add_argument(
        "--checkpoint-freq", type=int, default=50, help="Checkpoint frequency"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--simulate-failure", action="store_true", help="Simulate GPU failure"
    )
    parser.add_argument("--failure-gpu", type=int, default=1, help="Which GPU to fail")
    parser.add_argument(
        "--failure-iteration", type=int, default=100, help="When to fail"
    )
    ...
    mp.spawn(run_gemini_worker, args=(args.gpus, config), nprocs=args.gpus, join=True)
