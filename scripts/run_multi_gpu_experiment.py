...
def run_multi_gpu_gemini(num_gpus=4, iterations=100, checkpoint_freq=50):
    ...
    if results_path.exists():
        ...
        log_to_wandb({
            ...
            "multi_gpu/avg_checkpoint_ms": sum(checkpoint_times)/len(checkpoint_times) if checkpoint_times else 0
        })
    ...
    if results_path.exists():
        ...
        if recovery_events:
            for event in recovery_events:
                print(f"   - GPU {event.get('failed_rank')} recovered by GPU {event.get('recovered_by')}")
        ...
        log_to_wandb({
            ...
            "failure/recovery_time_ms": recovery_events[0].get('recovery_time_ms', 0) if recovery_events else 0
        })
        ...
        print("⚠️ Results file not found (may be expected if failure crashed the process)")
        return {'type': 'failure_simulation', 'total_time': total_time, 'note': 'process may have crashed'}
...
def compare_results(baseline, gemini_single, gemini_multi=None, failure_results=None):
    ...
    if baseline['avg_checkpoint_ms'] > 0 and gemini_single['avg_checkpoint_ms'] > 0:
        ...
        if gemini_multi and 'avg_checkpoint_ms' in gemini_multi and gemini_multi['avg_checkpoint_ms'] > 0:
            multi_speedup = baseline['avg_checkpoint_ms'] / gemini_multi['avg_checkpoint_ms']
        ...
        if failure_results and 'recovery_events' in failure_results:
            ...
            if events:
                ...
                print(f"   vs Disk recovery: ~{baseline['avg_checkpoint_ms'] * 2:.0f}+ ms (load + network)")
        ...
        if exp_logger:
            exp_logger.log_comparison(
                ...
                memory_load_time_ms=gemini_single['avg_checkpoint_ms'] * 0.5  # Estimate
            )
...
def save_final_results(baseline, gemini_single, gemini_multi, failure_results, comparison):
...
def main():
    ...
    try:
        ...
        # Compare results
        comparison = compare_results(baseline, gemini_single, gemini_multi, failure_results)
        
        # Save everything
        save_final_results(baseline, gemini_single, gemini_multi, failure_results, comparison)
